...
#include <string.h>
#include "ssdv.h"
//extern __declspec(dllexport) int sendImage(char imgid,char Callsign[],char mode,char *filein,char *fileout )
//{
void exit_usage()
{
...
/*	c = argc - optind;
	...
	*/
	...
	fin = fopen("Test.jpeg", "rb");
	fout = fopen("TestNow.out", "wb");
			if(!fin)
			{
				fprintf(stderr, "Error opening '%s' for input:\n", "Test.jpeg");
				perror("fopen");
